// This API is created by Nandhoman.nl. There're used a couple of package, those are listed below.
...
app.get("/recipes", (request, response) => {
  ...
  if (CommunityID === undefined) {
    ...
    pool.query(
      "SELECT * FROM recipes WHERE CommunityID =" + CommunityID,
      (error, result) => {
        console.log(2);
        if (error) throw error;
...
app.get("/keywordsintotal/:KeywordID", (req, response) => {
  ...
  pool.query(
    "SELECT * FROM keywordsintotal WHERE KeywordID =" + KeywordID,
    (error, result) => {
      if (error) throw error;
...
app.post("/setKeyword", function (req, res) {
  ...
  console.log(Values);
  pool.query(
    "INSERT INTO keywordsintotal VALUES" + String(Values),
    (error, result) => {
      if (error) throw error;
      else;
    }
